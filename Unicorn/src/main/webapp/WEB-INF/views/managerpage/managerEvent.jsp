...
<!-- 						<li class="menubar_item"><a href="">쿠폰 지급</a></li> -->
				...
				<div class="orderbar">
					...
					<div class="orderbar_title">이벤트 관리</div>
					<form action="managerEventDelete" method="post">
						<div class="orderbar_item">
							<table>
								...
								<!--for문 돌려서 제품 dto 꺼내기-->
								<c:forEach var="eventDTO" items="${data.list}">
									<tr id="events">
										<td class="table_eventNo">${eventDTO.eventNo}</td>
										<td class="table_eventTitle">${eventDTO.eventTitle}</td>
										<td class=""><input type="checkbox" name="eventNo"
											value="${eventDTO.eventNo}"></td>
									</tr>
								</c:forEach>
							</table>
						</div>
						<div class="edit_btn">
							<a href="managerEventInsert"><input class="write_btn"
								type="button" value="글쓰기"></a> <input class="delete_btn"
					...
					<div class="pageNum">
						...
						if (data != null) {
							...
							if (beginPaging == 1) {
								...
								out.println(
								"<a href='managerNotice?pageNum=" + (beginPaging - 1) + "&countPerPage=" + countPerPage + "'>[이전]</a>");
							}
		...
		document.addEventListener('DOMContentLoaded', function() {
		    ...
		    productRows.forEach(function(row) {
		        row.addEventListener('click', function(event) {
		            // 클릭한 요소가 td이고 클래스가 table_eventNo, table_eventTitle, 또는 table_noticeDate 인 경우에만 처리
		            if (event.target.tagName === 'TD' &&
		                (event.target.classList.contains('table_eventNo') ||
		                    event.target.classList.contains('table_eventTitle'))) {
		                // 클릭한 행에서 eventNo 가져오기
		                var eventNo = row.querySelector('.table_eventNo').innerText;
		                // eventNo 값을 파라미터로하여 managerEventUpdate 페이지로 리디렉션
		                window.location.href = 'managerEventUpdate?eventNo=' + eventNo;
		            }
		        });
		    });
