...
<head>
<meta charset="UTF-8">
<title>manager_event_update</title>
<style>
...
<!-- 						<li class="menubar_item"><a href="">쿠폰 지급</a></li> -->
				...
				<div class="orderbar">
					<div class="orderbar_title">이벤트 수정</div>

					<form action="/unicorn/managerEventDoUpdate" method="post">
						<div class="orderbar_item">
							<table>
								<tr>
									<td class="table_top">이벤트 번호</td>
									<td class="table_data">${event.eventNo}</td>
									<td><input class="tableEdit" type="hidden" name="eventNo"
										value="${event.eventNo}"></td>
								</tr>
								<tr>
									<td class="table_top">이벤트명</td>
									<td class="table_data">${event.eventTitle}</td>
									<td><input class="tableEdit" type="text" name="eventTitle"></td>
								</tr>
								<tr>
									<td class="table_top">이벤트 내용</td>
									<td class="table_data"><script>
										var eventDetails = "${event.eventDetails}";
										var chunkSize = 15;
										for (var i = 0, j = eventDetails.length; i < j; i += chunkSize) {
											document.write('<div>'
													+ eventDetails.slice(i, i
															+ chunkSize)
													+ '</div>');
										}
									</script></td>
									<td><input class="tableEdit" type="text"
										name="eventDetails"></td>
								</tr>
								<tr>
									<td class="table_top">이벤트 시작일</td>
									<td class="table_data">${event.eventStart}</td>
									<td><input class="tableEdit" type="date" name="eventStart"></td>
								<tr>
									<td class="table_top">이벤트 끝나는일</td>
									<td class="table_data">${event.eventEnd}</td>
									<td><input class="tableEdit" type="date" name="eventEnd"></td>
								</tr>
								<tr>
									<td class="table_top">이벤트 이미지</td>
									<td class="table_data"><script>
										var eventMainImg = "${event.eventMainImg}";
										var chunkSize = 15;
										for (var i = 0, j = eventMainImg.length; i < j; i += chunkSize) {
											document.write('<div>'
													+ eventMainImg.slice(i, i
															+ chunkSize)
													+ '</div>');
										}
									</script></td>
									<td><textarea class="tableEdit" id="notice_textarea"
											name="eventMainImg"></textarea>
								</tr>
							</table>
						</div>
