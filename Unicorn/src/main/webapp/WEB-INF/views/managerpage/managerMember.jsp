...
<!-- 						<li class="menubar_item"><a href="">쿠폰 지급</a></li> -->
				...
				<div class="orderbar">
					...
					<div class="orderbar_title">회원 관리</div>
					<form action="managerMemberDelete" method="post">
						<div class="orderbar_item">
	
							<table>
	
								<tr>
									<td class="table_top">회원번호</td>
									<td class="table_top">회원 아이디</td>
									<td class="table_top">회원 이름</td>
									<td class="table_top">비고</td>
								</tr>
								<!--for문 돌려서 제품 dto 꺼내기-->
								<c:forEach var="memberdto" items="${data.list}">
									<tr id="members">
										<td class="table_memberNo" >${memberdto.userno}</td>
										<td class="table_memberId">${memberdto.id}</td>
										<td class="table_memberName">${memberdto.name}</td>
										<td class=""><input type="checkbox" name="selectedUsers" value="${memberdto.userno}"></td>
									</tr>
								</c:forEach>
							</table>
					...
					<div class="pageNum">
						...
						if (data != null) {
							...
							if (beginPaging == 1) {
								...
								out.println("<a href='managerMember?pageNum=" + (beginPaging - 1) + "&countPerPage=" + countPerPage
								+ "'>[이전]</a>");
							}

							for (int i = beginPaging; i <= endPaging; i++) {
								if (i == pageNum) {
							out.println("<a href='managerMember?pageNum=" + i + "&countPerPage=" + countPerPage + "'><strong>[" + i
									+ "]</strong></a>");
								} else {
							out.println("<a href='managerMember?pageNum=" + i + "&countPerPage=" + countPerPage + "'>[" + i + "]</a>");
								}
							}
							...
							if (endPaging == totalPaging) {
								...
								out.println(
								"<a href='managerMember?pageNum=" + (endPaging + 1) + "&countPerPage=" + countPerPage + "'>[다음]</a>");
							}
						}
						%>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // 회원 행 요소 가져오기
        var productRows = document.querySelectorAll('#members');

        // 회원 행에 클릭 이벤트 리스너 추가
        productRows.forEach(function (row) {
            row.addEventListener('click', function (event) {
                // 클릭한 요소가 td이고 클래스가 table_memberNo 인 경우에만 처리
            	if (event.target.tagName === 'TD'&& (event.target.classList.contains('table_memberNo')
						|| event.target.classList.contains('table_memberId') 
						|| event.target.classList.contains('table_memberName'))) {
					// 클릭한 행에서 productNo 가져오기
					var memberNo = row.querySelector('.table_memberNo').innerText;
					// productNo 값을 파라미터로하여 managerProductUpdate 페이지로 리디렉션
					window.location.href = 'managerMemberDetails?memberNo='+ memberNo;}
				});
        });
    });
    document.addEventListener('DOMContentLoaded', function () {
