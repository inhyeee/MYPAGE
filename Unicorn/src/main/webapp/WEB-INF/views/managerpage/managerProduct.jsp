...
<!-- 						<li class="menubar_item"><a href="">쿠폰 지급</a></li> -->
				...
				<div class="orderbar">
					...
					<form action="managerProductDelete" method="post">
						<div class="orderbar_item">
							...
							<table>
								...
								<!--for문 돌려서 제품 dto 꺼내기-->
								<c:forEach var="productdto" items="${data.list}">
									<tr id="products">
										...
										<td class=""><input type="checkbox" name="productNo" value="${productdto.productNo}"></td>
									</tr>
								</c:forEach>
							</table>
					...
					<div class="pageNum">
						...
						if (data != null) {
							...
							if (beginPaging == 1) {
								...
								out.println("<a href='managerProduct?pageNum=" + (beginPaging - 1) + "&countPerPage=" + countPerPage
								+ "'>[이전]</a>");
							}

							for (int i = beginPaging; i <= endPaging; i++) {
								if (i == pageNum) {
							out.println("<a href='managerProduct?pageNum=" + i + "&countPerPage=" + countPerPage + "'><strong>[" + i
									+ "]</strong></a>");
								} else {
							out.println("<a href='managerProduct?pageNum=" + i + "&countPerPage=" + countPerPage + "'>[" + i + "]</a>");
								}
							}
							...
							if (endPaging == totalPaging) {
								...
								out.println(
								"<a href='managerProduct?pageNum=" + (endPaging + 1) + "&countPerPage=" + countPerPage + "'>[다음]</a>");
							}
						}
						%>
		...
		document.addEventListener(
						...
						function() {
							...
							productRows.forEach(function(row) {row.addEventListener('click',function(event) {
								// 클릭한 요소가 td이고 클래스가 table_produtNo, table_productName, 또는 table_productPrice 인 경우에만 처리
								if (event.target.tagName === 'TD'&& (event.target.classList.contains('table_produtNo')
										|| event.target.classList.contains('table_productName') 
										|| event.target.classList.contains('table_productPrice'))) {
									// 클릭한 행에서 productNo 가져오기
									var productNo = row.querySelector('.table_produtNo').innerText;
									// productNo 값을 파라미터로하여 managerProductUpdate 페이지로 리디렉션
									window.location.href = 'managerProductUpdate?productNo='+ productNo;}
								});
							});
						});
